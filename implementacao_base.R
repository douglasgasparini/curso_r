#Base.de.Moradores.PDAD <- read.csv2("~/curso_r/dados/Base-de-Moradores-PDAD.csv", stringsAsFactors=FALSE)
#Base_Domicilios_PDAD_2015 <- read_delim("http://www.codeplan.df.gov.br/wp-content/uploads/2018/02/Base_Domicilios_PDAD_2015.csv",";", escape_double = FALSE, trim_ws = TRUE)


library(data.table)

getwd()
setwd("/u01/u87253/curso_r")

# Carregar a base de um arquivo local
pdad_moradores <- data.table::fread("~/curso_r/dados/Base-de-Moradores-PDAD.csv",
                                    dec = ",",encoding = "Latin-1")

pdad_domicilios <- data.table::fread("http://www.codeplan.df.gov.br/wp-content/uploads/2018/02/Base_Domicilios_PDAD_2015.csv"
                                    ,dec = ",",encoding = "Latin-1")
...
#Os nomes das colunas (variáveis).
names(tabelas)

# Verificar a classe do objeto
class(tabelas) 

#Estrutura do data.frame. Mostra, entre outras coisas, as classes de cada coluna.
str(tabelas)
#mesma coisa mais organizada
dplyr::glimpse(tabelas)
# Consultar o número de linhas
nrow(tabelas)
# Consultar o número de colunas
ncol(tabelas)
...
#outra maneira
tabelas[,2]
# Selecionar apenas as duas primeiras linhas e colunas
tabelas[1:2,1:2]
# Selecionar apenas as duas primeiras linhas da coluna "TABLE_SCHEM"
tabelas[1:2,]$TABLE_SCHEM
# Selecionar linhar e colunas distintas
tabelas[c(1,7,10:12),c(1,3)]

# Tabular a coluna
table(tabelas$TABLE_SCHEM) 
table(tabelas[,2]) 
table(tabelas[,"TABLE_SCHEM"]) 
...
pdad_moradores <- RODBC::sqlQuery(db,
                                  "select A01_DOM_RA,A05_DOM_MORADORES from pdad.pdad_2015_mor")
...
# Carregar os pacotes necessários, O primeiro passo é criarmos o desenho inicial da pesquisa.
library(survey)
library(srvyr)
...
glimpse(pdad_domicilios) #sumariza
...
table(x$ANO) #conta os dados da tabela
...
pdad_domicilios %>%
  ...
  dplyr::mutate(DATA_CORRIGIDA=lubridate::mdy(DT_DATA_PESQUISA), #pacote e função
...
pdad_moradores <- 
  ...
  dplyr::mutate(RA=factor(A01_DOM_RA,
                             ...
                             labels=c('Brasília/Plano Piloto',      
                                      ...
                                      'Vicente Pires',
                                      'Fercal')))
#fazendo relatorio com inconsistencia 
inconsistencia <-
  # Utilizar a base de domicílios
  pdad_domicilios %>%
  # Selecionar a data da pesquisa
...
#5.4 Outras manipulações recorrentes
...
inflacao<-
  ...
  inflacao_long %>%
  tidyr::spread(Local,`Inflação`) # `questão de interpretação do codigo 
...
inflacao %>%
  ...
  ggplot(aes(x=ymd(referencia),y=`Inflação`/100,colour=Local))+ #hex color picker
...
esgotamento_grafico <-
  ...
  ggplot(aes(x=forcats::fct_reorder(esgotamento,-n),y=n,ymin=n_low,ymax=n_upp))+
  # Construir o gráfico de barras
  geom_bar(stat = "identity")+
  # Construir os erros
